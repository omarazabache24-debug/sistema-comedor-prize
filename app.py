...
{% if session.get('user') %}<div class="hero"><div><img src="{{url_for('static',filename='logo.png', v='4')}}" onerror="this.onerror=null;this.src='{{url_for('static',filename='logo.jpeg', v='4')}}';"></div><div><h1>Sistema Comedor PRIZE</h1><p class="muted">ERP para la Gestión del Comedor Corporativo</p><div class="checks"><div>✅ Registro y control de consumos</div><div>✅ Entrega de pedidos validando DNI</div><div>✅ Carga masiva de consumos Excel</div><div>✅ Cierre de día y reportes</div><div>✅ Envío automático por correo</div><div>✅ Roles y permisos</div></div></div><div class="users"><b>Usuarios de prueba</b><br>👤 admin / admin123<br>👥 rrhh / rrhh123<br>🍽️ comedor / comedor123</div></div><div class="layout"><aside class="side"><div class="brand"><img src="{{url_for('static',filename='logo.png', v='4')}}" onerror="this.onerror=null;this.src='{{url_for('static',filename='logo.jpeg', v='4')}}';"><h3>ERP Comedor</h3><small>{{session.get('user')}} · {{session.get('role')}}</small></div><nav class="nav"><a class="{{'on' if page=='dashboard'}}" href="{{url_for('dashboard')}}">📊 Dashboard</a><a class="{{'on' if page=='pedidos'}}" href="{{url_for('pedidos')}}">🍽️ Consumos</a><a class="{{'on' if page=='entregas'}}" href="{{url_for('entregas')}}">✅ Entregas</a><a class="{{'on' if page=='carga'}}" href="{{url_for('carga_masiva')}}">📥 Carga Masiva</a><a class="{{'on' if page=='trabajadores'}}" href="{{url_for('trabajadores')}}">👥 Trabajadores</a><a class="{{'on' if page=='cierre'}}" href="{{url_for('cierre_dia')}}">🔒 Cierre de Día</a><a href="{{url_for('logout')}}">🚪 Salir</a></nav></aside><main class="main">{% endif %}{% with messages=get_flashed_messages(with_categories=true) %}{% for c,m in messages %}<div class="flash {{c}}">{{m}}</div>{% endfor %}{% endwith %}{{content|safe}}{% if session.get('user') %}</main></div><div class="footer"><span>© 2026 Prize Superfruits - Comedor Corporativo</span><span>Versión 2.0.0</span></div>{% endif %}</body></html>
"""
...
def login():
    ...
    return render_page("""
    <div class="hero" style="margin:10px 10px 16px 10px;grid-template-columns:340px 1fr 300px;">
      <div style="border-right:1px solid var(--line);height:120px;display:flex;align-items:center;justify-content:center">
        <img src="/static/logo.png?v=4" onerror="this.onerror=null;this.src='/static/logo.jpeg?v=4';" style="max-width:260px;max-height:120px;object-fit:contain">
      </div>
      <div><h1>Sistema Comedor PRIZE</h1><p class="muted">ERP para la Gestión del Comedor Corporativo</p>
        <div class="checks"><div>✅ Registro y control de consumos</div><div>✅ Entrega de pedidos validando DNI</div><div>✅ Carga masiva de consumos Excel</div><div>✅ Cierre de día y reportes</div><div>✅ Envío automático por correo</div><div>✅ Roles y permisos</div></div>
      </div>
      <div class="users"><b>Usuarios de prueba</b><br>👤 admin / admin123<br>👥 rrhh / rrhh123<br>🍽️ comedor / comedor123</div>
    </div>
    <div style="display:grid;grid-template-columns:430px 1fr;gap:16px;margin:0 10px 10px 10px;align-items:stretch">
      <div class="card" style="position:relative;overflow:hidden;text-align:center;min-height:520px;padding:34px 38px;">
        <img src="/static/logo.png?v=4" onerror="this.onerror=null;this.src='/static/logo.jpeg?v=4';" style="width:250px;max-height:150px;object-fit:contain;margin-bottom:16px">
        <h2>Sistema Comedor PRIZE</h2><p class="muted">Acceso al sistema</p>
        <form method="post" style="text-align:left;margin-top:24px">
          <label><b>Usuario</b></label><input name="username" placeholder="Ingrese su usuario" required>
          <br><br><label><b>Clave</b></label><input name="password" type="password" placeholder="Ingrese su clave" required>
          <br><br><button style="width:100%">Ingresar</button>
        </form>
        <p class="mini" style="margin-top:26px">Usuarios de prueba:<br>admin/admin123 · rrhh/rrhh123 · comedor/comedor123</p>
        <div style="position:absolute;left:0;bottom:0;width:100%;height:70px;background:linear-gradient(135deg,#0b3b68 0%,#0b3b68 45%,#169b45 46%,#169b45 100%);clip-path:polygon(0 65%,100% 10%,100% 100%,0 100%)"></div>
      </div>
      <div class="card" style="min-height:520px">
        <h2 style="margin-top:0">Bienvenido al Sistema Comedor PRIZE</h2>
        <p class="muted">Interfaz preparada para Render y GitHub con las pestañas principales del sistema.</p>
        <div class="grid" style="margin-top:22px">
          <div class="card kpi"><b>✅</b><span>Validación por DNI</span></div>
          <div class="card kpi"><b>📥</b><span>Carga masiva Excel / Forms</span></div>
          <div class="card kpi"><b>🔒</b><span>Cierre diario</span></div>
          <div class="card kpi"><b>✉️</b><span>Reporte por correo</span></div>
        </div>
      </div>
    </div>
    """)
